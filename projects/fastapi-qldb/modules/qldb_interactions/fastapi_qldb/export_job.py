...
def describe_journal_export(ledger_name, export_id):
    ...
    logger.info("Let's describe a journal export for ledger with name: {}, exportId: {}.".format(ledger_name,
                                                                                                 export_id))
    export_result = qldb_client.describe_journal_s3_export(
...
def create_s3_bucket_if_not_exists(bucket_name, s3_resource):
    ...
    if not does_bucket_exists(bucket_name, s3_resource):
        ...
        try:
            ...
            logger.info(
                'Unable to create S3 bucket named: {}'.format(bucket_name))
            raise e
...
def create_export_role(role_name, key_arn, role_policy_name, s3_bucket):
    ...
    try:
        ...
        logger.info(
            'The role called {} does not exist. Creating it now.'.format(role_name))
        ...
        role_policy = POLICY_TEMPLATE.replace(
            '{statements}', role_policy_statement)

        create_policy_result = iam_client.create_policy(
            PolicyName=role_policy_name, PolicyDocument=role_policy)
        iam_client.attach_role_policy(
            RoleName=role_name, PolicyArn=create_policy_result.get('Policy').get('Arn'))
...
def create_export_and_wait_for_completion(name, bucket, prefix, encryption_config, role_arn=None):
    ...
    """
    if role_arn is None:
        role_arn = create_export_role(EXPORT_ROLE_NAME, encryption_config.get(
            'KmsKeyArn'), ROLE_POLICY_NAME, bucket)
    try:
        start_time = datetime.utcnow() - timedelta(minutes=JOURNAL_EXPORT_TIME_WINDOW_MINUTES)
        end_time = datetime.utcnow()

        result = create_export(name, start_time, end_time,
                               bucket, prefix, encryption_config, role_arn)
...
def create_export(ledger_name, start_time, end_time, s3_bucket_name, s3_prefix, encryption_configuration, role_arn):
    ...
    """
    logger.info(
        "Let's create a journal export for ledger with name: {}.".format(ledger_name))
    try:
...
def start_export_job(ledger_name=Constants.LEDGER_NAME):
    """
    ...
    #     # {}-{}".format(
    s3_bucket_name = "pragma-qldb-tutorial-journal-export-157098863241-dev"
    # Constants.JOURNAL_EXPORT_S3_BUCKET_NAME_PREFIX, identity['Account'])
